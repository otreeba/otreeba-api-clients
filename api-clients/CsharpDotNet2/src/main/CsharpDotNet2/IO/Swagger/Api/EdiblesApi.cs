...
namespace IO.Swagger.Api
{
    ...
    /// Represents a collection of functions to interact with the API endpoints
    /// </summary>
    public class EdiblesApi : IEdiblesApi
    {
        /// <summary>
        ...
        /// <returns></returns>
        public EdiblesApi(ApiClient apiClient = null)
        {
        ...
        public Edible GetEdibleByOcpc (string ocpc)
        {
            ...
            if (ocpc == null) throw new ApiException(400, "Missing required parameter 'ocpc' when calling GetEdibleByOcpc");
            ...
            if (((int)response.StatusCode) >= 400)
                throw new ApiException ((int)response.StatusCode, "Error calling GetEdibleByOcpc: " + response.Content, response.Content);
            else if (((int)response.StatusCode) == 0)
                throw new ApiException ((int)response.StatusCode, "Error calling GetEdibleByOcpc: " + response.ErrorMessage, response.ErrorMessage);
    
            return (Edible) ApiClient.Deserialize(response.Content, typeof(Edible), response.Headers);
        }
        ...
        /// <param name="sort">How to sort the items.</param> 
        /// <returns>InlineResponse2002</returns>            
        public InlineResponse2002 GetEdibles (int? page, int? count, string sort)
        {
 ...
 if (sort != null) queryParams.Add("sort", ApiClient.ParameterToString(sort)); // query parameter
            ...
            if (((int)response.StatusCode) >= 400)
                throw new ApiException ((int)response.StatusCode, "Error calling GetEdibles: " + response.Content, response.Content);
            else if (((int)response.StatusCode) == 0)
                throw new ApiException ((int)response.StatusCode, "Error calling GetEdibles: " + response.ErrorMessage, response.ErrorMessage);
