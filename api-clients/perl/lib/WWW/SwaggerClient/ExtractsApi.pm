...
#
package WWW::SwaggerClient::ExtractsApi;
...
# @param string $ocpc OCPC of the extract to return. (required)
{
    ...
    __PACKAGE__->method_documentation->{ 'get_extract_by_ocpc' } = { 
...
# @return Extract
#
sub get_extract_by_ocpc {
    my ($self, %args) = @_;

    # verify the required parameter 'ocpc' is set
    unless (exists $args{'ocpc'}) {
      croak("Missing the required parameter 'ocpc' when calling get_extract_by_ocpc");
    }
    ...
    my $_response_object = $self->{api_client}->deserialize('Extract', $response);
...
# @param string $sort How to sort the items. (optional, default to -createdAt)
{
    ...
    __PACKAGE__->method_documentation->{ 'get_extracts' } = { 
